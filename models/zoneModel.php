...
class zone extends DBManagerModel{
    ...
    public function getList($params = array()){
        ...
        $query = "SELECT   `zoneId`,`description`,`date_entered`, `display_name` AS `created_by`, created_by AS `created` 
    ...
    public function edit(){
        $this->updateRecord($this->entity(), $_POST, array("zoneId" => $_POST["zoneId"]));
    }
    public function del(){
        $this->delRecord($this->entity(), array("zoneId" => $_POST["id"]));
    }

    public function entity()
    {
        $data = array(
                        "tableName" => $this->pluginPrefix."zone"
                        ,"columnValidateEdit" => "created"
                        ,"entityConfig" => array("add" => true, "edit" => true, "del" => true, "view" => false)
                        ,"atributes" => array(                                        
                                        "zoneId" => array("type" => "int", "PK" => 0,"required" => false, "readOnly" => true, "autoIncrement" => true )                                        
